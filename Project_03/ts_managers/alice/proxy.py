...
class TupleSpaceAdapter:
    ...
    def _in(self, tupl, event_no):
        return self.ts._in(self.map_templates_out(tupl), event_no, None)

    def _inp(self, tupl, event_no):
        return self.ts._in(self.map_templates_out(tupl), event_no, 0)

    def _rd(self, tupl, event_no):
        return self.ts._rd(self.map_template_out(tupl), event_no, None)

    def _rdp(self, tupl, event_no):
        return self.ts._rd(self.map_template_out(tupl), event_no, 0)

    def _out(self, tupl, event_no):
        self.ts._out(tupl, event_no)

    def _reset(self, event_no):
        self.ts._reset(event_no)

    def _ready(self, lm_clock):
        self.ts._ready(lm_clock)